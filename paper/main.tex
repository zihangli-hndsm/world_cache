...
error. A source-only residual gate fit to a 90\% source-precision target
retains 22.1\% of target queries at 96.6\% target retained semantic accuracy and
achieves 89.5\% semantic mIoU in cross-scan label transfer. Cross-fitted
...
high-precision subset of cross-scan semantic correspondences?}
...
is scored against the hidden 3RScan transform only after matching. The
annotated subset contains 16 non-target source pairs and one target pair,
totaling 15,048 source and 1,069 target query records. The target is treated
as a single held-out diagnostic under this split, not as evidence of broad
generalization; all thresholds and selective choices are fit source-only.
...
ViT-G audit reaches 30.4\% weighted coverage and 85.0\% weighted semantic
precision.
...
pool it trades 11.2\%/89.5\% for 30.4\%/85.0\% coverage/precision.
...
\paragraph{Scene-level uncertainty.}
We resample the 16 source pairs rather than individual patches. The weighted
...
evidence of heterogeneous scene difficulty; the single held-out target is
reported as a point estimate.
A post-hoc RANSAC seed audit keeps the target stable at 93.1--93.5\% semantic
precision and 67.6--68.2\% coverage, but the source semantic aggregate ranges
...
0.040931 m, fit to a 90\% source-accuracy target, retains 22.1\% of target
queries at 96.6\% retained semantic accuracy. A source-selected frame-level reciprocal/margin control
reaches only 87.6\% at 12.1\% target coverage, so descriptor confidence alone
does not explain the consensus result. No selector reaches a 95\% precision constraint on the
full heterogeneous source pool. Leave-one-pair-out source-fit gates at the same
90\% source-accuracy target still yield 0.0\% and 10.0\% retained semantic accuracy on held-out 4aca
...
raises semantic mIoU to 89.5\% and instance mIoU to 90.7\% at 22.1\% coverage.
...
efficiency claim. The frozen headline snapshot contains 17 pairable RGB-D
sequence archives (16 sources and one target). The metadata lists many more
relations without the
corresponding frame archives; one target scene cannot establish broad
generalization. Applied without refitting, the source-only residual gate
...
effect was isolated. Spatial consensus makes a useful high-precision subset
explicit, but neither component eliminates candidate-availability,
